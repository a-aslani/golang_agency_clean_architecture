...
func (r *gateway) SaveDiscoverySession(ctx context.Context, obj *entity.DiscoverySession) error {
	r.log.Info(ctx, "called")
	...
	for _, file := range obj.Files {
		...
		const query = `INSERT INTO discovery_session_file (discovery_session_id, file_id) VALUES ($1, $2)`
...
func (r *gateway) ClearDiscoverySessionFileTable(ctx context.Context) error {
	_, err := r.db.ExecContext(ctx, `DELETE FROM discovery_session_file`)
	return err
}

func (r *gateway) ClearDiscoverySessionsTable(ctx context.Context) error {
	_, err := r.db.ExecContext(ctx, `DELETE FROM discovery_sessions`)
	return err
}
